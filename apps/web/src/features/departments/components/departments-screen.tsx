...
const LOGIN_HREF = "/login?next=%2Fdepartments";

/** `department.read` at organization scope OR department scope (a Department
 * Manager reads only their own department, which the API enforces). */
function hasDepartmentRead(
...
export function DepartmentsScreen() {
  ...
  const allowed = hasDepartmentRead(access.data?.grants);

  // Drop any department cache the moment the caller loses read access or their
  // session ends, so a re-grant starts from authoritative data.
  useEffect(() => {
    if (access.data === null || (access.isSuccess && !allowed)) {
      void client.cancelQueries({ queryKey: ["departments"] });
      client.removeQueries({ queryKey: ["departments"] });
    }
  ...
  return <DepartmentsManager key={access.data.userId} access={access.data} />;
}
