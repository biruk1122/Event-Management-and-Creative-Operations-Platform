...
describe("DepartmentsScreen access boundary", () => {
  it("renders the manager once department.read is confirmed", async () => {
    setup();
    expect(await screen.findByText("No departments yet")).toBeVisible();
  });
  ...
  it("drops to the session-expired state and clears department cache", async () => {
    const client = setup();
    ...
    expect(
      client.getQueryCache().findAll({ queryKey: ["departments"] }),
    ).toHaveLength(0);
  });
