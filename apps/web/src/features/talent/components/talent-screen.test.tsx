...
describe("TalentScreen access boundary", () => {
  ...
  it("renders the manager once an organization-scoped talent read grant is confirmed", async () => {
    setup();

    expect(await screen.findByText("0 talents")).toBeVisible();
  });

  it("denies a department-scoped reader (talent reads are organization-scoped)", async () => {
    currentAccess = {
      userId: "account-1",
  ...
  it("denies a caller with no talent read grant and never lists talent", async () => {
    currentAccess = {
      userId: "account-1",
      grants: [],
    ...
    expect(get).not.toHaveBeenCalledWith("/api/v1/talents", expect.anything());
  });
  ...
  it("shows the session-expired recovery path", async () => {
    ...
    ).toHaveAttribute("href", "/login?next=%2Ftalent");
    expect(
