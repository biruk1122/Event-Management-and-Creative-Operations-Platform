...
const LOGIN_HREF = "/login?next=%2Fteams";

/** `team.read` at organization scope OR department scope (a Department Manager
 * reads only their own department's teams, which the API enforces). */
function hasTeamRead(
...
export function TeamsScreen() {
  ...
  const allowed = hasTeamRead(access.data?.grants);

  // Drop any team cache the moment the caller loses read access or their
  // session ends, so a re-grant starts from authoritative data.
  useEffect(() => {
    if (access.data === null || (access.isSuccess && !allowed)) {
      void client.cancelQueries({ queryKey: ["teams"] });
      client.removeQueries({ queryKey: ["teams"] });
    }
  ...
  return <TeamsManager key={access.data.userId} access={access.data} />;
}
