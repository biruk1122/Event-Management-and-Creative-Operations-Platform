...
export function CampaignsScreen() {
  ...
  // Drop any campaign cache the moment the caller loses read access or their
  // session ends, so a re-grant starts from authoritative data.
  useEffect(() => {
    if (access.data === null || (access.isSuccess && !allowed)) {
      void client.cancelQueries({ queryKey: ["campaigns"] });
      client.removeQueries({ queryKey: ["campaigns"] });
    }
