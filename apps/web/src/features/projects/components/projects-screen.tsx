...
const LOGIN_HREF = "/login?next=%2Fprojects";

export function ProjectsScreen() {
  const access = useCurrentAccess();
  const client = useQueryClient();
  const allowed = access.data ? canReadProjects(access.data) : false;

  // Drop any project cache the moment the caller loses read access or their
  // session ends, so a re-grant starts from authoritative data.
  useEffect(() => {
    if (access.data === null || (access.isSuccess && !allowed)) {
      void client.cancelQueries({ queryKey: ["projects"] });
  ...
  return <ProjectsManager key={access.data.userId} access={access.data} />;
}
