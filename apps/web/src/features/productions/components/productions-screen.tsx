"use client";

import Link from "next/link";
import { useEffect } from "react";
import { useQueryClient } from "@tanstack/react-query";

import { Button } from "@/components/ui/button";
import { useCurrentAccess } from "@/features/auth/api/access-queries";

import { productionAbilities } from "../lib/production-access";
import { ProductionsManager } from "./productions-manager";

export function ProductionsScreen() {
  const access = useCurrentAccess();
  const client = useQueryClient();
  const allowed = access.data
    ? productionAbilities(access.data).canRead
    : false;

  useEffect(() => {
    if (access.data === null || (access.isSuccess && !allowed)) {
      void client.cancelQueries({ queryKey: ["productions"] });
      client.removeQueries({ queryKey: ["productions"] });
    }
  }, [access.data, access.isSuccess, allowed, client]);

  if (access.isPending) return <p role="status">Checking permissions…</p>;
  if (access.isError)
    return (
      <div role="alert" className="space-y-2">
        <p>{access.error.message}</p>
        <Button variant="outline" onClick={() => void access.refetch()}>
          Check permissions again
        </Button>
      </div>
    );
  if (access.data === null)
    return (
      <div role="alert" className="space-y-2">
        <p>Your session expired. Unsaved input remains in this tab.</p>
        <Link
          href="/login?next=%2Fprojects%2Fproduction"
          target="_blank"
          rel="noopener noreferrer"
          className="text-sm underline underline-offset-4"
        >
          Sign in in another tab
        </Link>
        <Button variant="outline" onClick={() => void access.refetch()}>
          I have signed in
        </Button>
      </div>
    );
  if (!allowed) return <p role="alert">You do not have access to this area.</p>;
  return <ProductionsManager key={access.data.userId} access={access.data} />;
}
