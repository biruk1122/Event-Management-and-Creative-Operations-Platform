...
- During a use case, events are **recorded into a transaction-scoped collector**, not dispatched
  ...
  transaction commits successfully**. When an event has a durable subscription, its envelope is
  also written to the PostgreSQL outbox in that transaction and relayed to durable handlers after
  commit. If the transaction rolls back, neither path publishes it.
...
| Durable     | Transactional **outbox** row written in the same transaction, then a poller relays and marks it done | At-least-once      | Notifications and approved side effects whose loss would be user-visible or unsafe |

An event may have consumers in both tiers. The direct dispatcher invokes only its best-effort
subscriptions; the outbox relay invokes only its durable subscriptions, so storing an event in the
outbox does not cause the same subscription to run through both paths.
...
- **Idempotent durable handlers.** Every durable handler with side effects is idempotent on
  `(eventId, handlerName)`. A processed marker or natural unique constraint is committed atomically
  with database side effects. An external call receives `eventId` as its idempotency key when the
  provider supports one; otherwise its duplicate risk and recovery procedure must be approved by
...
- Audit record shape (fields, not columns): `id` (UUID), `occurredAt` (UTC `timestamptz`),
  ...
  applicable, `outcome` (`succeeded`, `denied`, or `failed`), and `metadata` (JSON, minimal change
  summary only).
...
- **Audit record versus activity history.** Activity history is a general user-facing projection
  that can be rebuilt from domain events and is not the system of record for compliance. The audit
  table is the system of record and is exposed only through an authorized audit-log surface.
